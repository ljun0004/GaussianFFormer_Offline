...
class OccupancyLossOffline(BaseLoss):
    ...
    def forward(self, input_dict):
        ...
        for input_key, input_val in self.input_dict.items():
            actual_inputs.update({input_key: input_dict.get(input_val)})

        # 2. Extract the broken-down losses from our updated loss_voxel
        tot_loss, final_loss_dict = self.loss_voxel(**actual_inputs)

        # 3. Apply the parent weight and calculate the new weighted total
        weighted_dict = {}
        weighted_tot_loss = 0.0
        
        for k, v in final_loss_dict.items():
            weighted_val = self.weight * v
            weighted_dict[k] = weighted_val
            weighted_tot_loss = weighted_tot_loss + weighted_val
            
        # 4. Return the exact tuple train_offline.py expects!
        return weighted_tot_loss, weighted_dict
    ...
    def loss_voxel(self, pred_occ, sampled_xyz, sampled_label, occ_mask=None):
        ...
        for semantics in pred_occ:
            ...
            for k, v in loss_dict.items():
                loss = loss + v
                final_loss_dict[k] = final_loss_dict.get(k, 0.0) + (v / len(pred_occ))
            tot_loss = tot_loss + loss
            
        return tot_loss / len(pred_occ), final_loss_dict
